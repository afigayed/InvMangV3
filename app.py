...
def generate_data_qr(item_row):
    ...
    path = os.path.join(QR_DIR, f"{item_row['name']}_dataqr.png")
    qr.save(path)
    return path
...
def generate_name_image(name):
    ...
    path = os.path.join(PIC_DIR, f"{name}_genpic.png")
    img.save(path)
    return path
...
if submitted:
    ...
    if not name:
        ...
        if not existing.empty:
            ...
            barcode_path = os.path.join(QR_DIR, f"{name}_barcode.png")
            barcode_img.save(barcode_path)

            picture_path = ""
            if pic:
                ext = os.path.splitext(pic.name)[1]
                picture_path = os.path.join(PIC_DIR, f"{name}_pic{ext}")
...
if tool_input and tool_action:
    ...
    if result.empty:
        ...
        if tool_action == "Generate full-data QR code":
            ...
            path = os.path.join(QR_DIR, f"{row['name']}_nameqr.png")
            qr.save(path)
